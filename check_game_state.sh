...
LEAGUE_ID=$1
API_URL="https://draft.premierleague.com/api/game"
SAVED_FILE="game.json"
TEMP_FILE="/tmp/game_temp.json"
...
echo "Fetching game data from API..."
...
if ! diff -q "$TEMP_FILE" "$SAVED_FILE" > /dev/null 2>&1; then
    echo "✓ Changes detected in game status!"
