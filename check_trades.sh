...
LEAGUE_ID=$1
API_URL="https://draft.premierleague.com/api/draft/league/${LEAGUE_ID}/trades"
SAVED_FILE="${LEAGUE_ID}_data/league-${LEAGUE_ID}-trades.json"
TEMP_FILE="/tmp/trades_${LEAGUE_ID}_temp.json"
...
echo "Fetching trades data from API..."
...
if ! diff -q "$TEMP_FILE" "$SAVED_FILE" > /dev/null 2>&1; then
    echo "✓ Changes detected in trades!"
